...
// CETIS No. 45 - APP PRINCIPAL CORREGIDA
// ================================
...
function updateStats() {
    ...
    // Contar entradas (registros con horaEntrada)
    const entriesCount = todaysRecords.filter(record => record.horaEntrada).length;
    
    // Contar salidas (registros con horaSalida)
    const exitsCount = todaysRecords.filter(record => record.horaSalida).length;
    
    // Actualizar DOM directamente sin animación para evitar valores negativos
    if (totalStudentsEl) {
