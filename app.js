...
function setupEventListeners() {
    ...
    if (reportsBtn) reportsBtn.addEventListener('click', showReports);
...
async function startQRScan() {
    try {
        ...
        if (typeof jsQR === 'undefined') {
            alert('❌ Error: Librería QR no cargada. Verifica tu conexión a internet.');
            return;
        }
        ...
        if (error.name === 'NotAllowedError') {
            alert('❌ Permiso de cámara denegado. Por favor permite el acceso a la cámara y recarga la página.');
        } else if (error.name === 'NotFoundError') {
            alert('❌ No se encontró cámara. Verifica que tu dispositivo tenga cámara.');
        } else {
...
function processQRCode(qrData) {
    try {
        ...
        try {
            ...
            studentData = {
                matricula: qrData.substring(0, 20), // Tomar primeros 20 caracteres como matrícula
    ...
    recordsListEl.innerHTML = todaysRecords.map(record => `
        <div class="record-item">
            ...
            <div class="student-info">
                Matrícula: ${record.matricula || 'Sin matrícula'} | ${record.grado || 'N/A'}° ${record.grupo || 'N/A'}
            </div>
...
function updateStats() {
    ...
    if (totalEntriesEl) totalEntriesEl.textContent = entriesCount;
    if (totalExitsEl) totalExitsEl.textContent = exitsCount;
}
